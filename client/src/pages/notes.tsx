...
export default function Notes() {
  ...
  const generateQuizMutation = useMutation({
    mutationFn: async ({ noteId, questionCount, noteContent }: { noteId: string; questionCount?: number; noteContent?: string }) => {
      console.log("[NOTES AI] Sending POST /api/notes/:id/generate-quiz", { noteId, questionCount });
      const res = await apiRequest("POST", `/api/notes/${noteId}/generate-quiz`, { questionCount, noteContent });
    ...
    onSuccess: (data) => {
      ...
      // Add the new quiz to cache immediately for snappy UI updates.
      if (data?.quiz?.id) {
        queryClient.setQueryData(["/api/quizzes"], (oldQuizzes: any[] | undefined) => {
          const existing = oldQuizzes || [];
          if (existing.some((quiz) => quiz.id === data.quiz.id)) {
            return existing;
          }
          return [data.quiz, ...existing];
        });
      }

      // Invalidate and refetch quizzes to keep data fully in sync.
      queryClient.invalidateQueries({ queryKey: ["/api/quizzes"] });
    ...
    onError: (error: Error) => {
      ...
      setIsGenerating(false);
      const rawMessage = error.message || "Failed to generate quiz from note";
      const parsedMessage = (() => {
      ...
      const errorDescription = import.meta.env.DEV
        ? parsedMessage
        : "Could not generate a quiz from this note. Please try again.";
...
const handleAutoGenerate = () => {
    if (isGenerating || generateQuizMutation.isPending || generateFlashcardsMutation.isPending) {
      return;
    }
    ...
    if (autoGenerateType === "quiz") {
      ...
      generateQuizMutation.mutate({ noteId: selectedNoteId, questionCount: 5, noteContent });
    } else {
      if (!selectedDeckId) {
...
const insertFormat = (before: string, _after: string = "") => {
  ...
  Parameters:
  - prefix: Input consumed by this routine during execution
...
const addTag = () => {
  ...
  Parameters:
  - tag: Input consumed by this routine during execution
...
const handleDuplicateNote = () => {
  ...
  const currentNoteMeta = notes.find((note) => note.id === selectedNoteId);
      ...
      <div className={`flex min-w-0 flex-1 flex-col bg-card ${isMobile && showMobileSidebar ? 'hidden' : ''}`}>
        {selectedNoteId ? (
          <>
            ...
            {/* Contextual Next-Step Flow */}
            <div className="px-3 sm:px-6 py-3 border-b border-blue-100 dark:border-blue-900 bg-gradient-to-r from-blue-50/75 via-indigo-50/50 to-cyan-50/75 dark:from-slate-900 dark:via-indigo-950/25 dark:to-slate-900">
              <div className="rounded-xl border border-blue-200/80 dark:border-blue-800/70 bg-white/80 dark:bg-slate-900/70 p-3 sm:p-4 shadow-sm">
                <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
                  <div>
                    <p className="text-sm font-semibold text-foreground">Suggested next step</p>
                    <p className="text-xs text-muted-foreground">Test your understanding now and reinforce this note.</p>
                  </div>
                ...
                <div className="flex flex-wrap gap-2.5">
                  <Button
                    ...
                    disabled={!selectedNoteId}
                    className="gap-2 border-fuchsia-200 text-fuchsia-700 hover:bg-fuchsia-50 dark:border-fuchsia-800 dark:text-fuchsia-300 dark:hover:bg-fuchsia-950/30"
                    data-testid="button-flow-generate-quiz"
                  >
                  ...
                  <Button
                    ...
                    disabled={!selectedNoteId}
                    className="gap-2 border-cyan-200 text-cyan-700 hover:bg-cyan-50 dark:border-cyan-800 dark:text-cyan-300 dark:hover:bg-cyan-950/30"
                    data-testid="button-flow-create-flashcards"
                  >
                  ...
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => selectedNoteId && pinNoteMutation.mutate(selectedNoteId)}
                    disabled={!selectedNoteId || pinNoteMutation.isPending}
                    className="gap-2 border-amber-200 text-amber-700 hover:bg-amber-50 dark:border-amber-800 dark:text-amber-300 dark:hover:bg-amber-950/30"
                    data-testid="button-flow-mark-important"
                  >
                    <Pin className={`h-4 w-4 ${currentNoteMeta?.isPinned ? "fill-current" : ""}`} />
                    {currentNoteMeta?.isPinned ? "Marked Important" : "Mark as Important"}
                  </Button>
                  ...
                  <Button
                    ...
                    className="gap-2 text-blue-700 hover:bg-blue-100 dark:text-blue-300 dark:hover:bg-blue-950/40"
                    data-testid="button-flow-test-understanding"
                  >
